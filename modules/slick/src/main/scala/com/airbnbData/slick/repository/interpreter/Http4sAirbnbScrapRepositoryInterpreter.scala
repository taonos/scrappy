...
class Http4sAirbnbScrapRepositoryInterpreter extends AirbnbScrapRepository {
  ...
  private def searchUri: Uri = {
    getUri("https://api.airbnb.com/v2/search_results")
      ...
      .+*?(MinBeds())
      .+*?(Sort())
    //      .withQueryParam(QueryParam[ClientId].key, QueryParamEncoder[ClientId].encode(ClientId()))
    //        .withQueryParam()
    //      .withQueryParam(ClientId.queryParamInstance)
    //      .withQueryParam(Locale.queryParamInstance)
    //      .withQueryParam(Currency.queryParamInstance)
    //      .withQueryParam(Limit.queryParamInstance)
    //      .withQueryParam(Offset.queryParamInstance)
    //      .withQueryParam(FetchFacets.queryParamInstance)
    //      .withQueryParam(Guests.queryParamInstance)
    //      .withQueryParam(Ib.queryParamInstance)
    //      .withQueryParam(Location.queryParamInstance)
    //      .withQueryParam(Neighborhoods.queryParamInstance)
    //      .withQueryParam(MinBathrooms.queryParamInstance)
    //      .withQueryParam(MinBedrooms.queryParamInstance)
    //      .withQueryParam(MinBeds.queryParamInstance)
    //      .withQueryParam(Sort.queryParamInstance)
  }
...
//      .withQueryParam(Locale.queryParamInstance)
//      .withQueryParam(Currency.queryParamInstance)
//      .withQueryParam(Format.queryParamInstance)
//      .withQueryParam(Source.queryParamInstance)
//      .withQueryParam("number_of_guests", guests)
////    .withQueryParam(NumberOfGuests(guests))
  }
